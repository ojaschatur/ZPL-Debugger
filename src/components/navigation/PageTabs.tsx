export type PageType = 'request' | 'preview';
...
export function PageTabs({ activePage, onPageChange }: PageTabsProps) {
    return (
        <div className="flex gap-2 p-1 bg-white/40 rounded-2xl border border-[var(--glass-border)] backdrop-blur-sm">
            <button
                className={`flex-1 flex items-center justify-center gap-2 px-6 py-3 rounded-xl font-medium text-sm transition-all duration-300 ${activePage === 'request'
            ...
            <button
                className={`flex-1 flex items-center justify-center gap-2 px-6 py-3 rounded-xl font-medium text-sm transition-all duration-300 ${activePage === 'preview'
                        ? 'bg-gradient-to-r from-accent-primary to-accent-secondary text-white shadow-glow'
                        : 'text-[var(--text-secondary)] hover:text-[var(--text-primary)] hover:bg-white/50'
                    }`}
